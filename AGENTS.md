...
**本项目处于 Phase 0（模块化单体 → 未来微服务）。** 代码正在从集中位置迁移到独立服务目录。
...
packages/contracts/  ←  所有共享类型的单一真理源（HTTP/Event/DTO）
        ↑
packages/features/   ←  定义 trait + 类型，不得包含实现，不得依赖 usecases
        ↑
packages/core/       ←  系统级抽象（TenantId/Error/Config/Clock）+ usecase 实现
        ↑
...
**Phase 0 当前状态（过渡期，Agent 必须知道代码实际在哪里）**：

| 目标位置 | 当前实际位置 | 说明 |
|----------|-------------|------|
| `services/user/` 等领域服务 | `packages/core/usecases/` | 业务逻辑集中在 usecases 包 |
| BFF 组合层 | `servers/api/` | 单体 Axum 服务器，聚合所有路由（尚未按端拆分） |
| `services/event-bus/` | `packages/core/usecases/` 内间接使用 | event-bus 目录是 stub |
| 6 个 future feature | `packages/features/` 中的 README stub | feed, notifications, payments, profile, social-graph, wallet |

**关于 `servers/` 与 `apps/bff/` 的关系**：
ARCHITECTURE.md 中设计的是 `apps/bff/`（每端一个 BFF），但实际项目中 BFF 的职责由 `servers/` 承担。
`servers/api/` 当前是单体聚合（所有路由在一个二进制），未来拆分为 `servers/web/`、`servers/admin/` 等。
**BFF 是后端组合层，不是前端的一部分。**

**生成新代码时的规则**：
- 新增**业务模块** → 应该在 `services/<domain>/` 下创建（即使目前是 stub，也要按目标结构生成）
- 新增**共享 trait/类型** → `packages/features/<domain>/` 或 `packages/core/`
- 新增**基础设施实现** → `packages/adapters/`
- 新增**HTTP 端点** → `servers/api/src/routes/`
...
- **SvelteKit 不承载领域真相**：UI 组合 + 瞬时状态，业务逻辑在 `usecases/`
