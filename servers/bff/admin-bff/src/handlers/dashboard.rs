...
use admin_service::ports::{
    CounterRepository as AdminCounterRepo, TenantRepository as AdminTenantRepo, TenantSummary,
};
...
pub async fn fetch_dashboard(
    db: EmbeddedTurso,
) -> Result<DashboardView, crate::error::AdminBffError> {
    // Create repositories
    ...
    let tenants = tenant_repo_for_list.list_tenants().await.map_err(|e| {
        crate::error::AdminBffError::Internal(format!("Failed to list tenants: {}", e))
    })?;

    let recent_tenants: Vec<TenantSummaryView> = tenants
        .into_iter()
        .take(10)
